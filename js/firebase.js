// ── Firebase + Tarefas + Email (Google Apps Script)
...
import { initializeApp } from "https://www.gstatic.com/firebasejs/11.6.1/firebase-app.js";
  ...
  // ── Google Apps Script — envio de email via btocteam@gmail.com
  const GAS_URL = 'https://script.google.com/macros/s/AKfycbxW2uEia6odxOWXxmMv6qJ9OzY338FCYBwY7QtZmvU1sl4snWpYg4FA6qV37-7NZ2c0/exec';
  const EMAILS = { 'Tiago Fachada': null, 'Tiago Pereira': null, 'Miguel Leitão': 'miguel.leitao@btocnet.com', 'Vânia Costa': null };

  async function sendEmailGAS(to, subject, html) {
    if (!to) return;
    try {
      await fetch(GAS_URL, {
        method: 'POST',
  ...
  async function sendEmailConcluida(creatorName, task, client, assignees, deadline, notes) {
    ...
    const subject = `✅ Tarefa concluída — ${client}`;
    const html = `
      <div style="font-family:sans-serif;max-width:500px;margin:0 auto;">
        <h2 style="color:#18181b;">✅ Tarefa Concluída</h2>
        <table style="width:100%;border-collapse:collapse;">
          <tr><td style="padding:8px;color:#71717a;font-size:12px;text-transform:uppercase;">Cliente</td><td style="padding:8px;font-weight:600;">${client}</td></tr>
          <tr style="background:#f4f4f5;"><td style="padding:8px;color:#71717a;font-size:12px;text-transform:uppercase;">Tarefa</td><td style="padding:8px;">${task}</td></tr>
          <tr><td style="padding:8px;color:#71717a;font-size:12px;text-transform:uppercase;">Concluída por</td><td style="padding:8px;">${assigneeStr}</td></tr>
          <tr style="background:#f4f4f5;"><td style="padding:8px;color:#71717a;font-size:12px;text-transform:uppercase;">Data</td><td style="padding:8px;">${new Date().toLocaleDateString('pt-PT',{day:'2-digit',month:'long',year:'numeric'})}</td></tr>
          ${notes ? `<tr><td style="padding:8px;color:#71717a;font-size:12px;text-transform:uppercase;">Notas</td><td style="padding:8px;">${notes}</td></tr>` : ''}
        </table>
        <p style="margin-top:16px;font-size:12px;color:#71717a;">BTOC TEAM — Gestão de Tarefas</p>
      </div>`;
    await sendEmailGAS(to, subject, html);
  }

  async function sendEmail(name, task, client, deadline, notes) {
    const to = EMAILS[name];
    if (!to) return;
    const subject = `📋 Nova tarefa atribuída — ${client}`;
    const html = `
      <div style="font-family:sans-serif;max-width:500px;margin:0 auto;">
        <h2 style="color:#18181b;">📋 Nova Tarefa Atribuída</h2>
        <table style="width:100%;border-collapse:collapse;">
          <tr><td style="padding:8px;color:#71717a;font-size:12px;text-transform:uppercase;">Cliente</td><td style="padding:8px;font-weight:600;">${client}</td></tr>
          <tr style="background:#f4f4f5;"><td style="padding:8px;color:#71717a;font-size:12px;text-transform:uppercase;">Tarefa</td><td style="padding:8px;">${task}</td></tr>
          <tr><td style="padding:8px;color:#71717a;font-size:12px;text-transform:uppercase;">Responsável</td><td style="padding:8px;">${name}</td></tr>
          <tr style="background:#f4f4f5;"><td style="padding:8px;color:#71717a;font-size:12px;text-transform:uppercase;">Prazo</td><td style="padding:8px;">${deadline || 'Sem data'}</td></tr>
          ${notes ? `<tr><td style="padding:8px;color:#71717a;font-size:12px;text-transform:uppercase;">Notas</td><td style="padding:8px;">${notes}</td></tr>` : ''}
        </table>
        <p style="margin-top:16px;font-size:12px;color:#71717a;">BTOC TEAM — Gestão de Tarefas</p>
      </div>`;
